	...
	public function bricks_template_posts_custom_column( $column, $post_id ) {
		...
		if ( $column === 'template_thumbnail' ) {
			...
			if ( has_post_thumbnail( $post_id ) ) {
				echo '<a href="' . get_the_permalink( $post_id ) . '" target="_blank">' . get_the_post_thumbnail( $post_id, 'thumbnail' ) . '</a>';
			}
			...
			// Get automatically-generated template screenshot from custom directory (@since 1.10)
				...
				if ( $latest_file ) {
					...
					echo '<a href="' . get_the_permalink( $post_id ) . '" target="_blank">' . "<img src=\"$file_url\" /></a>";
				} else {
					echo '-';
				}
			}
