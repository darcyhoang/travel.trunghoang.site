...
class Query_Filters_Indexer {
	...
	public function indexer_is_running() {
		...
		// If the next_job time is more than 30 minutes (abnormal), set the indexer status to false, so the job can continue again (maybe orphan job in previous run)
		if ( $flag && $next_job ) {
			$job_created_at = strtotime( $next_job['job_created_at'] );
			$now            = time();
			$diff           = $now - $job_created_at;
